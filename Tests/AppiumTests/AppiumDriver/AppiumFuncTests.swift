...
class AppiumFuncTests: XCTestCase {
    ...
    override func setUp() {
        ...
        let opts = [
            ...
            DesiredCapabilities.app: "\(packageRootPath)/AppiumFuncTests/app/UICatalog.app.zip",
    ...
    func testCanGetElementEnabled() {
        let result = try! homeScreen.buttonsBtn()?.isEnabled() ?? false
        XCTAssertTrue(result)
    }

    func testCanGetElementDisplayed() {
        let result = try! homeScreen.buttonsBtn()?.isDisplayed() ?? false
        XCTAssertTrue(result)
    }
